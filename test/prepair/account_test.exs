defmodule Prepair.AccountTest do
  ...
  describe "get_user_by_email_and_password/2" do
    test "does not return the user if the email does not exist" do
      refute Account.get_user_by_email_and_password(
               "unknown@example.com",
               "hello world!"
             )
    end
    ...
    test "returns the user if the email and password are valid" do
      ...
      assert %User{id: ^id} =
               Account.get_user_by_email_and_password(
                 user.email,
                 valid_user_password()
               )
    end
  ...
  describe "register_user/1" do
    ...
    test "validates email and password when given" do
      {:error, changeset} =
        Account.register_user(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} =
        Account.register_user(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} =
        Account.register_user(%{email: String.upcase(email)})
  ...
  describe "apply_user_email/3" do
    ...
    test "requires email to change", %{user: user} do
      {:error, changeset} =
        Account.apply_user_email(user, valid_user_password(), %{})
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        Account.apply_user_email(user, password, %{email: email})
  ...
  describe "update_user_email/2" do
    ...
    test "does not update email if user email changed", %{
      ...
      assert Account.update_user_email(
  ...
  describe "update_user_password/3" do
    ...
    test "updates the password", %{user: user} do
      ...
      assert Account.get_user_by_email_and_password(
               user.email,
               "new valid password"
             )
    end
  ...
  describe "reset_user_password/2" do
    ...
    test "validates maximum values for password for security", %{user: user} do
      ...
      {:error, changeset} =
        Account.reset_user_password(user, %{password: too_long})
    ...
    test "updates the password", %{user: user} do
      {:ok, updated_user} =
        Account.reset_user_password(user, %{password: "new valid password"})
